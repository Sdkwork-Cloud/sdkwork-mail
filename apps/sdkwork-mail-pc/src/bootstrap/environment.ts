...
export function resolveEnvironment(): MailEnvironment {
  ...
  return {
    apiBaseUrl: normalizeBaseUrl(
      import.meta.env.VITE_sdkwork_mail_PC_APP_API_BASE_URL,
      deriveAppApiBaseUrl(applicationPublicHttpUrl),
    ),
    appbaseAppApiBaseUrl: normalizeBaseUrl(
      import.meta.env.VITE_sdkwork_mail_PC_APPBASE_APP_API_BASE_URL,
      deriveAppApiBaseUrl(applicationPublicHttpUrl),
    ),
    backendApiBaseUrl: normalizeBaseUrl(
      import.meta.env.VITE_sdkwork_mail_PC_BACKEND_API_BASE_URL,
      deriveBackendApiBaseUrl(applicationPublicHttpUrl),
    ),
    appbaseLoginUrl: normalizeBaseUrl(
      import.meta.env.VITE_sdkwork_mail_PC_APPBASE_LOGIN_URL,
      "http://127.0.0.1:3900",
    ),
