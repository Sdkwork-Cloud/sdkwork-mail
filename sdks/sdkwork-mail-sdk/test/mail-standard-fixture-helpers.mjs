...
} from '../bin/Mail-standard-shared-helpers.mjs';
import {
  mail_FLUTTER_REQUIRED_STANDARD_FILES,
  mail_ROOT_REQUIRED_CONTRACT_FILES,
  mail_TYPESCRIPT_PROVIDER_PACKAGE_ROOT_README,
...
export function buildMailVerifierFixtureFileList(assembly) {
  const filesToCopy = [
    ...mail_ROOT_REQUIRED_CONTRACT_FILES,
  ...
  for (const language of assembly.officialLanguages ?? []) {
    filesToCopy.push(`sdkwork-mail-sdk-${language}/README.md`);
  }
  ...
  for (const provider of assembly.providers ?? []) {
    filesToCopy.push(`sdkwork-mail-sdk-typescript/providers/Mail-sdk-provider-${provider.providerKey}/README.md`);
    filesToCopy.push(`sdkwork-mail-sdk-typescript/providers/Mail-sdk-provider-${provider.providerKey}/index.js`);
    filesToCopy.push(`sdkwork-mail-sdk-typescript/providers/Mail-sdk-provider-${provider.providerKey}/index.d.ts`);
    filesToCopy.push(`sdkwork-mail-sdk-typescript/providers/Mail-sdk-provider-${provider.providerKey}/package.json`);
  }

  return normalizeStringArray(filesToCopy);
}

export function createMailVerifierFixture(
  mutator,
  workspaceRoot = resolveMailSdkWorkspaceRoot(import.meta.url),
) {
