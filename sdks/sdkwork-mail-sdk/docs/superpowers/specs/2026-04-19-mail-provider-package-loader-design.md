# Mail Provider Package Loader Design

## Summary

`sdkwork-mail-sdk` already standardizes provider metadata, package boundaries, capability catalogs,
...
### Option C: Put dynamic import logic directly into `MailDriverManager`
...
`MailDriverManager` remains the registration and selection authority. A new provider package loader
...
Add `sdkwork-mail-sdk-typescript/src/provider-package-loader.ts`.
...
5. Register via existing `registerMailProviderModule(...)` or
   `registerMailProviderModules(...)`.
...
- `resolveMailProviderPackageLoadTarget(...)`
...
After this change, `sdkwork-mail-sdk` no longer stops at “provider packages exist.” It will define
the complete reference-standard path for package-boundary Mail adapter loading:
